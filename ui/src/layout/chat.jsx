...
const [message, setMessage] = useState("");
  ...
  <div key={index} className="flex items-start space-x-4">
    ...
    <div>
      <div className="flex items-center space-x-2">
        <h3 className="font-bold text-indigo-600">{msg.data}</h3>
        <div className="inline-flex items-center rounded-full border px-2.5 py-0.5 w-fit text-xs font-semibold transition-colors focus:outline-none focus:ring-2 focus:ring-ring focus:ring-offset-2 border-transparent hover:bg-primary/80 bg-blue-200 text-blue-800">
          {msg.timeStamp}
        </div>
      </div>
      <p className="text-gray-700">{msg.data.body}</p>
    </div>
  </div>
))}
              </div>
            ...
            <div className="flex w-full items-center space-x-2">
              ...
              />
              <button onClick={(event) => send()} className="inline-flex items-center justify-center rounded-md text-sm font-medium ring-offset-background transition-colors focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2 disabled:pointer-events-none disabled:opacity-50 h-10 px-4 py-2 bg-indigo-600 hover:bg-indigo-700 text-white">
                Send
              </button>
            </div>
          </div>
